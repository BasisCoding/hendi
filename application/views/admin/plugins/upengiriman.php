...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#table-pengiriman').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('admin/MasterData/get_pengiriman')?>",
                "type": "POST"
            },
		...
		$('#form-addPengiriman').submit(function() {
			...
			if ($('[name="konf_password"]').val() != $('[name="password"]').val()) {
            	...
            	actionData(formData, 'addPengiriman');
            	reload_table();
		...
		$('#form-updatePengiriman').submit(function() {
        	...
        	actionData(formData, 'updatePengiriman');
        	reload_table();

        	return false;
		});

		$('#form-deletePengiriman').submit(function() {
			var formData = new FormData();
            formData.append('id', $('[name="id_pengiriman_delete"]').val()); 
			
			actionData(formData, 'deletePengiriman');
        	reload_table();

        	return false;
		});

		$('#table-data-pengiriman').on('click', '.delete-data', function() {
			var id = $(this).attr('data-id');
			var nama = $(this).attr('data-nama');
			
			$('#pengiriman-delete').html(nama);
			$('[name="id_pengiriman_delete"]').val(id);

			$('#modal-deletePengiriman').modal('show');
		});

		$('#table-data-pengiriman').on('click', '.edit-data', function() {
			var id = $(this).attr('data-id');
			$.ajax({
				url: '<?= base_url("admin/MasterData/getPengirimanById") ?>',
				type: 'POST',
				dataType: 'JSON',
				data:{id:id},
				success:function (data) {
					...
					$('#modal-updatePengiriman').modal('show');
				}
			});
		});
